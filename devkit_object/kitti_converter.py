...
def load_dataset(work_num=1, split='train'):
    ...
    for i in range(work_num):
        ...
        labels += l
        Ks += k
    path_shape = os.path.join(root, out_dir, 'shape_{}.npy'.format(split))
    path_label = os.path.join(root, out_dir, 'label_{}.npy'.format(split))
    path_k = os.path.join(root, out_dir, 'k_{}.npy'.format(split))
